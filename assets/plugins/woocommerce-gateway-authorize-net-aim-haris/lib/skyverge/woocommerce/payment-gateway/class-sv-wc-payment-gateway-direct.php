...
abstract class SV_WC_Payment_Gateway_Direct extends SV_WC_Payment_Gateway {
	...
	protected function validate_credit_card_fields( $is_valid ) {
		...
		/*$account_number   = SV_WC_Helper::get_post( 'billing_creditcard' );
		$expiration_month = SV_WC_Helper::get_post( 'billing_expirymm' );
		$expiration_year  = SV_WC_Helper::get_post( 'billing_expiryyy' );
		$csc              = SV_WC_Helper::get_post( 'billing_cvv' );*/
		
		$account_number   =  $_POST['billing_creditcard'];
		$expiration_month =  $_POST['billing_expirymm'];
		$expiration_year  =  $_POST['billing_expiryyy'];
		$csc              =  $_POST['billing_cvv'];
	...
	protected function get_order( $order_id ) {
		...
		// payment info
		if ( SV_WC_Helper::get_post( 'billing_creditcard' ) && ! SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) ) {

			// common attributes
			$order->payment->account_number = str_replace( array( ' ', '-' ), '', SV_WC_Helper::get_post( 'billing_creditcard' ) );

			if ( $this->is_credit_card_gateway() ) {

				// credit card specific attributes
				$order->payment->card_type      = SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-card-type' );
				$order->payment->exp_month      = SV_WC_Helper::get_post( 'billing_expirymm' );
				$order->payment->exp_year       = SV_WC_Helper::get_post( 'billing_expiryyy' );

				if ( $this->csc_enabled() ) {
					$order->payment->csc        = SV_WC_Helper::get_post( 'billing_cvv' );
				}
