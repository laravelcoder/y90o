...
<table class="autotship">
 ...
 <?php   global  $wpdb;
// mysql_connect('naturesfix.chxw3sngrvwf.us-east-1.rds.amazonaws.com', 'natfixdbu', 'c5uqEzuy44') or trigger_error("Unable to connect to the database: " . mysql_error()); 
//mysql_select_db('nfixprodb') or trigger_error("Unable to switch to the database: " . mysql_error()); 
...
//      include('config.php');  // include your code to connect to DB. bloodrynane aa
        ...
        echo $tbl_name1= $wpdb->base_prefix."postmeta";         
...
mirror it here.
        */
       echo  $query = "SELECT COUNT(*) as num FROM ".$tbl_name1." WHERE meta_key='_payment_method' AND meta_value='paypal' ORDER BY post_id DESC";
 ...
 //first item to display on this page
        ...
        /* Get data. */
       echo  $sql = "SELECT * FROM $tbl_name1 WHERE meta_key='_payment_method' AND meta_value='paypal' ORDER BY post_id DESC LIMIT $start, $limit";
...
?>    
    <tr>
        ...
        <td><?php $sql3 = mysql_query("SELECT * FROM ".$wpdb->base_prefix."term_relationships a, ".$wpdb->base_prefix."terms b WHERE a.object_id='$post_ids' AND a.term_taxonomy_id=b.term_id") or die(mysql_error());
...
<strong style='color:#0074A2;'>Expiry:</strong><br/>
<?php 
if($custom_fields['_billing_expirymmyy']!=""){  
$my_custom_field = $custom_fields['_billing_expirymmyy'];
foreach ( $my_custom_field as $key => $billing_expiry_mmyys1) ?>
<?php echo " ".$billing_expiry_mmyys1.""; }?><br>
